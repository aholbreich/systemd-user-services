...
Item {
  ...
  // Unit name currently mid-action, so the row can show a spinner/disable
  // its buttons instead of racing a second command against the first.
  ...
  Process {
    ...
    onExited: function(exitCode) {
      if (exitCode !== 0) {
        var text = String(actionStderr.text || actionStdout.text || "")
        root.lastError = (root.pendingVerb + " " + root.pendingUnit + " failed: " + text).trim()
      }
      root.pendingUnit = ""
      root.pendingVerb = ""
